...
void PhotoProcessor::scanPhotos(const QStringList& filePaths, PhotoListModel* model)
{
...
void PhotoProcessor::processPhotos(PhotoListModel* model, const ProcessingSettings& settings)
{
    if (m_trackpoints.isEmpty()) {
    ...
    for (int i = 0; i < model->count(); ++i) {
        ...
        // Get photo timestamp
        auto timestamp = ExifHandler::getPhotoTimestamp(photo.filePath, timeOffsetSeconds);
        ...
        if (!settings.dryRun) {
            bool writeSuccess = ExifHandler::writeGpsData(
                photo.filePath, lat, lon, elevation);
    ...
    qInfo() << "Processing complete:" << successCount << "/" << model->count() << "photos updated";
    emit processingComplete(successCount, model->count());
}

void PhotoProcessor::stopProcessing()
{
